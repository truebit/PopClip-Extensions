...
class Translator:
    ...
    def translate(self, source, ip_addr=None):
        json5 = self._get_json5_from_google(source, ip_addr)
    ...
    def _get_json5_from_google(self, source, ip_addr=None):
        escaped_source = quote(source, '')
        host = str(ip_addr) if ip_addr else "translate.google.com"
    ...
    def _unescape(self, text):
        return re_sub(r"\\.?", lambda x:eval('"%s"'%x.group(0)), text)
